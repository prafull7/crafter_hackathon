...
class ItemView:
    ...
    def _item(self, canvas, index, item, unit):
        ...
        # Allow aliasing inventory item names to specific textures for display.
        # If an apricot texture exists, use it to represent the generic 'food' item.
        display_name = item
        try:
            if item == "food" and "apricot" in self._textures._originals:
                display_name = "apricot"
        except AttributeError:
            # Fallback to default item name if texture originals are not accessible.
            display_name = item
        texture = self._textures.get(display_name, 0.8 * unit)
